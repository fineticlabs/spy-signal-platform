...
# ── Backfill EOD data ────────────────────────────────────────────────────────
echo ""
echo "[$(date '+%H:%M:%S')] Running EOD data backfill (last 2 days)..."
python scripts/backfill_data.py --days 2 || {
    echo "WARNING: EOD backfill failed."
}

# ── Replay today's trading day ────────────────────────────────────────────────
echo ""
echo "[$(date '+%H:%M:%S')] Replaying today ($TODAY)..."
REPLAY_OUTPUT=$(python scripts/replay_day.py --date "$TODAY" 2>&1) || true
echo "$REPLAY_OUTPUT"
